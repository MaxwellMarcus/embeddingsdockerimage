...
class MedCLIPModel(nn.Module):
    ...
    def from_pretrained(self, input_dir=None):
        ...
        state_dict = torch.load(os.path.join(input_dir, constants.WEIGHTS_NAME), map_location=torch.device( "cuda" ) if torch.cuda.is_available() else torch.device( "cpu" ) )
        self.load_state_dict(state_dict)
    ...
    def encode_text(self, input_ids=None, attention_mask=None):
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.to( torch.device( "cuda" ) if torch.cuda.is_available() else torch.device( "cpu" ) )
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        if attention_mask is not None:
            attention_mask = attention_mask.to( torch.device( "cuda" ) if torch.cuda.is_available() else torch.device( "cpu" ) )
        pixel_values = pixel_values.to( torch.device( "cuda" ) if torch.cuda.is_available() else torch.device( "cpu" ) )
...
class PromptClassifier(nn.Module):
    ...
    def forward(self, pixel_values=None, prompt_inputs=None, **kwargs):
        ...
        for cls_name, cls_text in prompt_inputs.items():
            ...
            for k in cls_text.keys(): inputs[k] = cls_text[k].to( torch.device( "cuda" ) if torch.cuda.is_available() else torch.device( "cpu" ) )
    ...
    def forward(self,
        ...
        **kwargs,
        ...
        if labels is not None and return_loss:
            labels = labels.to( torch.device( "cuda" ) if torch.cuda.is_available() else torch.device( "cpu" ) ).float()
