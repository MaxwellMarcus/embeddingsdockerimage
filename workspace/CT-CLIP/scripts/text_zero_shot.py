...
class CTClipTextInference(nn.Module):
    def __init__(
        self,
        CTClip: CTCLIP,
        *,
        ...
        self.text_ds = pd.read_csv( metadata )
        ...
        if True:
            with torch.no_grad():
                ...
                for model, filename in models_to_evaluate:
                    ...
                    for i in range( self.row_min, self.row_max, 1 ):
                        metadata = self.text_ds.loc[ i ]
                        s = ": ".join( ";;;".join( metadata.to_string( header=True ).split( "\n" ) ).split() )
                        s = s.replace( ";;;", "\n" )
                        print( "Text: ", s )
                        print( "Length: ", len( s ) )
                        text_tokens=self.tokenizer(
                                        s, return_tensors="pt", padding="max_length", truncation=True, max_length=512).to(device)
                        output = model( text_tokens, None, return_latents = True, device = device )
                        filename = metadata.loc[ "VolumeName" ].replace( ".nii.gz", ".metadata.npy" )

                        np.save( f"{self.result_folder_txt}{filename}", output.detach().cpu().numpy() )
